...
{
    [ExportCodeFixProvider(LanguageNames.CSharp, Name = nameof(ReplaceEnumeratorWithForEachCodeFixProvider))]
    [Shared]
    public class ReplaceEnumeratorWithForEachCodeFixProvider : BaseCodeFixProvider
    {
        public sealed override ImmutableArray<string> FixableDiagnosticIds
        {
            get { return ImmutableArray.Create(DiagnosticIdentifiers.ReplaceEnumeratorWithForEach); }
        }
        ...
        public sealed override async Task RegisterCodeFixesAsync(CodeFixContext context)
        {
            ...
            foreach (Diagnostic diagnostic in context.Diagnostics)
            {
                ...
                {
                    case DiagnosticIdentifiers.ReplaceEnumeratorWithForEach:
                        {
                            CodeAction codeAction = CodeAction.Create(
                                "Replace enumerator with foreach",
            ...
            CancellationToken cancellationToken)
        {
            ...
            var rewriter = new EnumeratorCurrentRewriter(
        ...
        private class EnumeratorCurrentRewriter : CSharpSyntaxRewriter
        {
            ...
            public EnumeratorCurrentRewriter(ISymbol symbol, string name, string newName, SemanticModel semanticModel, CancellationToken cancellationToken)
            {
                _symbol = symbol;
                _name = name;
